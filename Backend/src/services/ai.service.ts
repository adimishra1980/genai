...
type GenerateInterviewReportParams = {
  resume: string;
...
async function generateInterviewReport({
  ...
  const report: InterviewReport = JSON.parse(response.text);
  return report;
}
...
type ResumePdf = z.infer<typeof resumePdfSchema>;
...
async function generateResumePDF({
  ...
  `;
    ...
    const report: ResumePdf = JSON.parse(response.text);
